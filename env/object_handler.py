...
class ObjectHandler:
    ...
    def update(self):
        ...
        [npc.update() for npc in self.npc_list]
         ...
         # check for dead NPCs and update kill count
        for npc in self.npc_list:
            if not npc.alive and not npc.counted:  # if NPC is dead and hasn't been counted yet
                self.kill_count += 1  # increment kill count
                npc.counted = True  # mark NPC as counted
